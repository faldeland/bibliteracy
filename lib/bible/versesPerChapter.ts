// Canonical KJV verses-per-chapter for every book of the Protestant 66-book
// canon. Used to:
//   • validate parsed verse references (verseStart / verseEnd in range)
//   • cross-check the chapter counts in `books.ts`
//   • drive the chapter/verse pickers in BibleReader
//
// Source: the KJV verse counts published by the American Bible Society and
// reflected verbatim in every major public-domain KJV edition (e.g. the
// Project Gutenberg KJV, ETCBC, openbible.info). These numbers are
// universally agreed and the dataset has not changed since 1769.
//
// The arrays are zero-indexed by chapter-number minus one:
//   VERSES_PER_CHAPTER["Gen"][0] === 31  // Genesis 1 has 31 verses
//
// Total verses: 31,102 (KJV). This is asserted in tests/bible/books.test.ts.
...
export function chapterCount(bookId: string): number | null {
  const arr = VERSES_PER_CHAPTER[bookId];
  return arr ? arr.length : null;
}

export function verseCount(bookId: string, chapter: number): number | null {
  const arr = VERSES_PER_CHAPTER[bookId];
  if (!arr) return null;
  if (!Number.isInteger(chapter) || chapter < 1 || chapter > arr.length) {
    return null;
  }
  return arr[chapter - 1];
}

export const TOTAL_KJV_VERSES: number = (() => {
  let n = 0;
  for (const arr of Object.values(VERSES_PER_CHAPTER)) {
